...
Flyway must be applied through V14 before purchases are enabled. Open `/stars`, confirm all five packages and a native `XTR` invoice, and use a controlled purchase to verify one charge creates one Credit ledger entry even after webhook retry. Run `/paysupport` and confirm the request reaches only the configured private admin chat; verify `/ask`, `/reject`, and a controlled `/refund` notify the player and reverse Credits.
