...
enum class EquipmentActionStatus { SUCCESS, LOCKED, NOT_FOUND, INSUFFICIENT_RESOURCES, MAX_LEVEL, GROUP_FULL, LAST_UNIT }
...
class InventoryService(
    ...
    fun army(telegramId: Long): Army {
        val commanderLevel = jdbc.sql("SELECT commander_level FROM players WHERE telegram_id = :id")
        ...
        val membership = jdbc.sql(
            "SELECT group_id, player_unit_id FROM battle_group_units WHERE group_id IN (SELECT id FROM battle_groups WHERE player_telegram_id = :id)",
        ).param("id", telegramId).query { rs, _ ->
        ...
        return Army(commanderLevel, cpLimit(commanderLevel), groups, units)
    }
    ...
    fun battleSnapshot(army: Army): CombatGroupSnapshot {
        ...
        return CombatGroupSnapshot(
            ...
            units = group.units.map { owned ->
                val definition = catalog.require(owned.code)
                val stats = definition.stats.scaled(owned.level)
                UnitBattleSnapshot(
                    id = owned.id,
                    code = owned.code,
                    level = owned.level,
    ...
    @Transactional
    fun acquire(telegramId: Long, code: String, craft: Boolean): EquipmentAction {
        ...
        val credits = if (craft) definition.craftCredits else definition.buyCredits
        val materials = if (craft) definition.craftMaterials else 0
        ...
        recordWallet(telegramId, "CREDITS", -credits.toLong(), origin, unitId)
        if (materials > 0) recordWallet(telegramId, "MATERIALS", -materials.toLong(), origin, unitId)
        recordEquipment(telegramId, unitId, origin, -credits.toLong(), -materials.toLong(), null, 1)
        return EquipmentAction(EquipmentActionStatus.SUCCESS, OwnedUnit(unitId, definition.code, 1, 100, origin), definition)
    }
    ...
    fun upgrade(telegramId: Long, unitId: UUID): EquipmentAction {
        ...
        return EquipmentAction(EquipmentActionStatus.SUCCESS, next, definition)
    }
    ...
    fun toggleInActiveGroup(telegramId: Long, unitId: UUID): EquipmentAction {
        ...
        if (!selected) {
            ...
            val slot = (1..12).first { candidate ->
                jdbc.sql("SELECT COUNT(*) FROM battle_group_units WHERE group_id = :groupId AND slot_no = :slot")
                    .param("groupId", group.id).param("slot", candidate).query(Int::class.java).single() == 0
            }
    ...
    companion object {
        fun cpLimit(commanderLevel: Int): Int = (10 + (commanderLevel.coerceIn(1, 50) - 1) / 10).coerceAtMost(14)
    }
}
