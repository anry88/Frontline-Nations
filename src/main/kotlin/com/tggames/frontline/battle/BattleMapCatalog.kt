...
class BattleMapCatalog(objectMapper: ObjectMapper) {
    val maps: List<BattleMapDefinition> = ClassPathResource("catalog/battle-maps.json").inputStream.use {
        objectMapper.readValue(it, object : TypeReference<List<BattleMapDefinition>>() {})
    }.also(::validate)

    fun forBiome(biome: String): BattleMapDefinition =
        (maps.firstOrNull { biome in it.biomes } ?: maps.first { it.id == "open-front" }).resolvedFor(biome)
