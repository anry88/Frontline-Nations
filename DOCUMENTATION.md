...
11. After a week resolves, `/front` presents the latest completed result and replay together with the next open matchup and map; `/contribute` immediately targets that next week. Campaign status, controls, results, event descriptions, map objectives, formation types, equipment names, ratings, and help copy use the player's selected locale across all eight supported languages.
